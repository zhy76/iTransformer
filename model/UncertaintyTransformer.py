...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.decoder = Decoder(
            ...
            norm_layer=torch.nn.LayerNorm(configs.d_model),
            projection=nn.Linear(configs.d_model, configs.c_out * 2, bias=True)  # c_out * 2 是为了同时预测均值和方差
        )
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        mu = dec_out[:, -self.pred_len:, :self.c_out]
        sigma = F.softplus(dec_out[:, -self.pred_len:, self.c_out:])
        return mu, sigma
